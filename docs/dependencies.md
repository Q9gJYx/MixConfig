...
- **Pre-extracted configurations and energy statistics** for the MNIST demo are shipped at `data/mnist_70k_configs.npz`. These are *outputs* of running our Python port on public data, with no IP overlap with the upstream algorithm code.
...
| Pre-extracted configurations | MixConfig authors | Released here, see `data/` | MIT (this repo) |
